...
void VWinSockDriver::Listen (bool state) {
  ...
  if (state) {
    ...
    if (net_acceptsocket == -1) {
      ...
      if (net_acceptsocket == -1) Sys_Error("WINS_Listen: Unable to open accept socket\n");
    }
  } else {
    // disable listening
    if (net_acceptsocket != -1) {
...
int VWinSockDriver::OpenSocket (int port) {
  ...
  if (ioctlsocket(newsocket, FIONBIO, &trueval) == -1) goto ErrorReturn;
...
int VWinSockDriver::CheckNewConnections () {
  ...
  if (recvfrom(net_acceptsocket, buf, sizeof(buf), MSG_PEEK, nullptr, nullptr) >= 0) return net_acceptsocket;
  return -1;
  unguard;
}
...
//
//==========================================================================
int VWinSockDriver::Write (int socket, const vuint8 *buf, int len, sockaddr_t *addr) {
...
//
//==========================================================================
int VWinSockDriver::Broadcast (int socket, const vuint8 *buf, int len) {
...
//
//==========================================================================
int VWinSockDriver::PartialIPAddress (const char *in, sockaddr_t *hostaddr, int DefaultPort) {
...
//
//==========================================================================
int VWinSockDriver::GetAddrFromName (const char *name, sockaddr_t *addr, int DefaultPort) {
...
//
//==========================================================================
int VWinSockDriver::AddrCompare (const sockaddr_t *addr1, const sockaddr_t *addr2) {
...
//
//==========================================================================
int VWinSockDriver::GetSocketPort (const sockaddr_t *addr) {
