  ...
  if (BspVis[sid8]&bvbit) { \
    ...
    if (r_advlight_opt_optimise_scissor) { \
      ...
      for (const subregion_t *region = vsub->regions; region; region = region->next) { \
        ...
        else if (region->realfloor) UpdateBBoxWithSurface(LitBBox, region->realfloor->surfs, &region->realfloor->texinfo, curreg->efloor.splane->SkyBox, true); \
        ...
        else if (region->realceil) UpdateBBoxWithSurface(LitBBox, region->realceil->surfs, &region->realceil->texinfo, curreg->eceiling.splane->SkyBox, true); \
      } \
    } \
  } \
} while (0)
