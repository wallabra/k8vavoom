...
private:
  ...
  void AddDecal (TVec org, const VName &dectype, line_t *li, int level);
  void AddDecalById (TVec org, int id, line_t *li, int level);
  // called by `AddDecal()`
  void AddOneDecal (int level, TVec org, VDecalDef *dec, line_t *li);
  void PutDecalAtLine (int tex, float orgz, float lineofs, VDecalDef *dec, line_t *li, vuint32 flips);
