...
//
//==========================================================================
bool VLevel::CheckPlane (linetrace_t &Trace, const sec_plane_t *Plane, unsigned rev) const {
  ...
  Trace.HitPlaneNormal = (rev ? -Plane->normal : Plane->normal);
...
bool VLevel::CheckPlanes (linetrace_t &Trace, sector_t *Sec) const {
  ...
  if (StartReg != nullptr) {
    for (sec_region_t *Reg = StartReg; Reg != nullptr; Reg = Reg->next) {
      if (!CheckPlane(Trace, Reg->efloor, Reg->regflags&sec_region_t::RF_FlipFloor)) return false; // hit floor
      if (!CheckPlane(Trace, Reg->eceiling, Reg->regflags&sec_region_t::RF_FlipCeiling)) return false; // hit ceiling
    }
...
//
//==========================================================================
static bool SightCheckPlane (const SightTraceInfo &Trace, const sec_plane_t *Plane, unsigned rev) {
