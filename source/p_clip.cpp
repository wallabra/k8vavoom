...
bool VViewClipper::ClipIsBBoxVisible (const float BBox[6], bool checkFrustum) const {
  ...
  if (checkFrustum && clip_frustum && clip_frustum_sub && Frustum.isValid()) {
    if (clip_frustum_only_back) {
      ...
      if (!Frustum.checkBox(BBox)) return false;
    }
  }
...
void VViewClipper::CheckAddClipSeg (const seg_t *seg, const TPlane *Mirror, bool skipSphereCheck) {
  ...
  if (!skipSphereCheck && clip_frustum && clip_frustum_sub && !CheckSegFrustum(seg)) {
...
void VViewClipper::CheckLightAddClipSeg (const seg_t *seg, const TVec &CurrLightPos,
                                         const float CurrLightRadius, const TPlane *Mirror, bool skipSphereCheck)
{
