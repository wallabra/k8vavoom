...
vuint32 M_LookupColourName (const VStr &Name); // returns 0 if not found (otherwise high bit is set)
...
static inline __attribute__((unused)) vuint32 bjprng_ranval (BJPRNGCtx *x) {
...
static inline __attribute__((unused)) void bjprng_raninit (BJPRNGCtx *x, vuint32 seed) {
  x->a = 0xf1ea5eed;
  x->b = x->c = x->d = seed;
  for (unsigned i = 0; i < 20; ++i) (void)bjprng_ranval(x);
}
