  ...
  for (int i = 0; i < files.length(); ++i) {
    ...
    if (fi.lumpName != NAME_None) {
      if (!VStr::isLowerCase(*fi.lumpName)) {
        GLog.WriteLine(NAME_Warning, "Archive \"%s\" contains non-lowercase lump name '%s'", *getArchiveName(), *fi.lumpName);
      }
    } else {
...
void VFileDirectory::buildNameMaps (bool rebuilding) {
  bool doReports = false; //(rebuilding ? false : !fsys_no_dup_reports);
  if (doReports) {
    ...
    if (fi.fileName.length()) {
      if (doReports) {
        if (aszip && filemap.has(fi.fileName)) {
          GLog.WriteLine(NAME_Warning, "duplicate file \"%s\" in archive \"%s\".", *fi.fileName, *getArchiveName());
          GLog.WriteLine(NAME_Warning, "THIS IS FUCKIN' WRONG. DO NOT USE BROKEN TOOLS TO CREATE PK3 FILES!");
        } else if (f > 0) {
          for (int pidx = f-1; pidx >= 0; --pidx) {
            if (files[pidx].fileName.length()) {
              if (files[pidx].fileName == fi.fileName) {
                ...
                GLog.WriteLine(NAME_Warning, "duplicate file \"%s\" in archive \"%s\".", *fi.fileName, *getArchiveName());
                GLog.WriteLine(NAME_Warning, "THIS IS FUCKIN' WRONG. DO NOT USE BROKEN TOOLS TO CREATE WAD FILES!");
              }
              break;
            }
          }
...
//==========================================================================
//
//  VPakFileBase::LumpNamespace
//
//==========================================================================
EWadNamespace VPakFileBase::LumpNamespace (int Lump) {
  if (Lump < 0 || Lump >= pakdir.files.length()) return WADNS_Global;
  return (EWadNamespace)pakdir.files[Lump].lumpNamespace;
}
