...
void TVisBuilder::SimpleFlood (portal_t *srcportal, int leafnum) {
  ...
  for (int i = 0; i < leaf->numportals; ++i) {
    ...
    if (!portalsee[p-portals]) continue;
...
void TVisBuilder::CheckStack (leaf_t *leaf, threaddata_t *thread) {
  for (pstack_t *p = thread->pstack_head.next; p; p = p->next) {
    if (p->leaf == leaf) throw GLVisError("CheckStack: leaf recursion");
  }
}
...
//
//==========================================================================
winding_t *TVisBuilder::ClipWinding (winding_t *in, TPlane2D *split) {
  double dists[2];
  TVec2D mid;
  winding_t *neww;
  ...
  if (dists[0] >= -ON_EPSILON && dists[1] >= -ON_EPSILON) return in;

  neww = new winding_t;

  // generate a split point
  TVec2D &p1 = in->points[0];
  TVec2D &p2 = in->points[1];
  double dot = dists[0]/(dists[0]-dists[1]);

  // avoid round off error when possible
  { // fuck you, gcc!
       if (split->normal.x == 1) mid.x = split->dist;
  else if (split->normal.x == -1) mid.x = -split->dist;
  else mid.x = p1.x+dot*(p2.x-p1.x);
  }

  { // fuck you, gcc!
       if (split->normal.y == 1) mid.y = split->dist;
  else if (split->normal.y == -1) mid.y = -split->dist;
  else mid.y = p1.y+dot*(p2.y-p1.y);
  }
...
//  Generates separating planes canidates by taking two points from source
//  and one point from pass, and clips target by them.
//  If target is totally clipped away, that portal can not be seen through.
//
//==========================================================================
winding_t *TVisBuilder::ClipToSeperators (winding_t *source, winding_t *pass, winding_t *target) {
  int i, j;
  TPlane2D plane;
  TVec2D v2;
  double d;
  ...
  for (i = 0; i < 2; ++i) {
    ...
    for (j = 0; j < 2; ++j) {
      ...
      length = plane.normal.x*plane.normal.x+plane.normal.y*plane.normal.y;
      ...
      if (d > ON_EPSILON) {
        // source is on the positive side, so we want all pass and target on the negative side
        // flip the normal if the source portal is backwards
        plane.normal = -plane.normal;
        plane.dist = -plane.dist;
      } else if (d >= -ON_EPSILON) {
        // coplanar with source portal
        continue;
      }
      ...
      if (d <= ON_EPSILON) {
        // coplanar with seperating plane, or points on negative side, so not a seperating plane
        continue;
      }

      // clip target by the seperating plane
      target = ClipWinding(target, &plane);
      if (!target) return nullptr; // target is not visible
    }
  }
...
//
//==========================================================================
void TVisBuilder::RecursiveLeafFlow (int leafnum, threaddata_t *thread, pstack_t *prevstack, int StackDepth) {
  pstack_t stack;
  portal_t *p;
  TPlane2D backplane;
  ...
  for (i = 0; i < leaf->numportals; ++i) {
    ...
    if (prevstack->portalplane.normal == backplane.normal) continue; // can't go out a coplanar face
...
void TVisBuilder::CalcPortalVis () {
  ...
  for (;;) {
    ...
    portal_t *p = GetNextPortal();
    if (!p) break;

    PortalFlow(p);

    if (Owner.verbose) Owner.DisplayMessage("portal:%4i  mightsee:%4i  cansee:%4i\n", (int)(p - portals), p->nummightsee, p->numcansee);
    ++i;
  }
...
void TVisBuilder::LeafFlow (int leafnum) {
  ...
  if (outbuffer[leafnum>>3]&(1<<(leafnum&7))) throw GLVisError("Leaf portals saw into leaf");
...
void TVisBuilder::BuildPVS () {
  ...
  for (int i = 0; i < numsubsectors; ++i) LeafFlow(i);
