...
void ZopfliInitCache(size_t blocksize, ZopfliLongestMatchCache* lmc) {
  ...
  if(lmc->sublen == NULL) {
    fprintf(stderr,
        "Error: Out of memory. Tried allocating %u bytes of memory.\n",
        (unsigned)(ZOPFLI_CACHE_LENGTH * 3 * blocksize));
    exit (EXIT_FAILURE);
  }
...
void ZopfliCacheToSublen(const ZopfliLongestMatchCache* lmc,
  ...
  for (j = 0; j < ZOPFLI_CACHE_LENGTH; j++) {
    unsigned lenn = cache[j * 3] + 3;
    unsigned dist = cache[j * 3 + 1] + 256 * cache[j * 3 + 2];
    for (i = prevlength; i <= lenn; i++) {
      sublen[i] = dist;
    }
    if (lenn == maxlength) break;
    prevlength = lenn + 1;
  }
}
