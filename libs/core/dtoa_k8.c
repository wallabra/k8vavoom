...
/* Configuration and modifications specific to compliation for k8VaVoom */
 ...
 * #define DTOA_MALLOC your_malloc, where your_malloc(n) acts like malloc(n)
 *	if memory is available and otherwise does something you deem
 *	appropriate.  If DTOA_MALLOC is undefined, malloc will be invoked
 *	directly -- and assumed always to succeed.  Similarly, if you
 *	want something other than the system's free() to be called to
 *	recycle memory acquired from DTOA_MALLOC, #define DTOA_FREE to be the
 *	name of the alternate routine.  (DTOA_FREE or free is only called in
 ...
 *	suffices to get rid of DTOA_MALLOC calls except for unusual cases,
 ...
 *	Such a call invokes DTOA_REALLOC (assumed to be "realloc" if DTOA_REALLOC
 ...
 *	Sudden_Underflow.
 * #define DTOA_USE_LOCALE to use the current locale's decimal_point value.
...
#endif
#ifdef DTOA_MALLOC
extern void *DTOA_MALLOC(size_t);
#else
#define DTOA_MALLOC Z_Malloc
#endif

#ifdef DTOA_REALLOC
extern void *DTOA_REALLOC(void*,size_t);
#else
#define DTOA_REALLOC Z_Realloc
#endif

#ifndef DTOA_FREE
#define DTOA_FREE Z_Free
#endif
...
#ifndef K8_SKIP_DECLARATIONS
#ifdef __cplusplus
extern "C" double strtodEx(const char *s00, char **se);
#ifndef STRTOD_ONLY
...
set_max_dtoa_threads(unsigned int n)
{
	...
	if (n > maxthreads) {
		L = n*sizeof(ThInfo);
		if (TI1) {
			TI1 = (ThInfo*)DTOA_REALLOC(TI1, L);
			memset(TI1 + maxthreads, 0, (n-maxthreads)*sizeof(ThInfo));
			}
		else {
			TI1 = (ThInfo*)DTOA_MALLOC(L);
			if (TI0_used) {
...
Balloc(int k MTd)
{
	...
	if (k <= Kmax && (rv = freelist[k]))
		...
		rv = (Bigint *)DTOA_MALLOC(sizeof(Bigint) + (x-1)*sizeof(ULong));
#else
...
gethex( const char **sp, U *rvp, int rounding, int sign MTd)
{
	...
	if (!(s0 = decimalpoint_cache)) {
		...
		if ((decimalpoint_cache = (unsigned char*)
				DTOA_MALLOC(strlen((const char*)s0) + 1))) {
...
retlow1:
 ...
 double
strtodEx(const char *s00, char **se)
{
