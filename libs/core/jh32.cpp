...
//static void E8(JHCtx *state);   /*The bijective function E8, in bitslice form */
//static void F8(JHCtx *state);   /*The compression function F8 */
...
static void E8(JHCtx *state)
{
      vuint32 i,j,roundnumber,temp0;

      /*perform 42 rounds*/
      for (roundnumber = 0; roundnumber < 42; roundnumber = roundnumber+7) {
            /*round 7*roundnumber+0: Sbox, MDS and swapping layer*/
            for (i = 0; i < 4; i++) {
                  Sbox(state->x[0][i],state->x[2][i], state->x[4][i], state->x[6][i],((vuint32*)E8_bitslice_roundconstant[roundnumber])[i] );
                  Sbox(state->x[1][i],state->x[3][i], state->x[5][i], state->x[7][i],((vuint32*)E8_bitslice_roundconstant[roundnumber])[i+4] );
            ...
            for (i = 0; i < 4; i++) {
                  Sbox(state->x[0][i],state->x[2][i], state->x[4][i], state->x[6][i],((vuint32*)E8_bitslice_roundconstant[roundnumber+1])[i] );
                  Sbox(state->x[1][i],state->x[3][i], state->x[5][i], state->x[7][i],((vuint32*)E8_bitslice_roundconstant[roundnumber+1])[i+4] );
                  L(state->x[0][i],state->x[2][i],state->x[4][i],state->x[6][i],state->x[1][i],state->x[3][i],state->x[5][i],state->x[7][i]);
            ...
            for (i = 0; i < 4; i++) {
                  Sbox(state->x[0][i],state->x[2][i], state->x[4][i], state->x[6][i],((vuint32*)E8_bitslice_roundconstant[roundnumber+2])[i] );
                  Sbox(state->x[1][i],state->x[3][i], state->x[5][i], state->x[7][i],((vuint32*)E8_bitslice_roundconstant[roundnumber+2])[i+4] );
                  L(state->x[0][i],state->x[2][i],state->x[4][i],state->x[6][i],state->x[1][i],state->x[3][i],state->x[5][i],state->x[7][i]);
            ...
            for (i = 0; i < 4; i++) {
                  Sbox(state->x[0][i],state->x[2][i], state->x[4][i], state->x[6][i],((vuint32*)E8_bitslice_roundconstant[roundnumber+3])[i] );
                  Sbox(state->x[1][i],state->x[3][i], state->x[5][i], state->x[7][i],((vuint32*)E8_bitslice_roundconstant[roundnumber+3])[i+4] );
                  L(state->x[0][i],state->x[2][i],state->x[4][i],state->x[6][i],state->x[1][i],state->x[3][i],state->x[5][i],state->x[7][i]);
            ...
            for (i = 0; i < 4; i++) {
                  Sbox(state->x[0][i],state->x[2][i], state->x[4][i], state->x[6][i],((vuint32*)E8_bitslice_roundconstant[roundnumber+4])[i] );
                  Sbox(state->x[1][i],state->x[3][i], state->x[5][i], state->x[7][i],((vuint32*)E8_bitslice_roundconstant[roundnumber+4])[i+4] );
                  L(state->x[0][i],state->x[2][i],state->x[4][i],state->x[6][i],state->x[1][i],state->x[3][i],state->x[5][i],state->x[7][i]);
            ...
            for (i = 0; i < 4; i++) {
                  Sbox(state->x[0][i],state->x[2][i], state->x[4][i], state->x[6][i],((vuint32*)E8_bitslice_roundconstant[roundnumber+5])[i] );
                  Sbox(state->x[1][i],state->x[3][i], state->x[5][i], state->x[7][i],((vuint32*)E8_bitslice_roundconstant[roundnumber+5])[i+4] );
                  L(state->x[0][i],state->x[2][i],state->x[4][i],state->x[6][i],state->x[1][i],state->x[3][i],state->x[5][i],state->x[7][i]);
            }
            ...
            for (i = 0; i < 4; i++) {
                  Sbox(state->x[0][i],state->x[2][i], state->x[4][i], state->x[6][i],((vuint32*)E8_bitslice_roundconstant[roundnumber+6])[i] );
                  Sbox(state->x[1][i],state->x[3][i], state->x[5][i], state->x[7][i],((vuint32*)E8_bitslice_roundconstant[roundnumber+6])[i+4] );
                  L(state->x[0][i],state->x[2][i],state->x[4][i],state->x[6][i],state->x[1][i],state->x[3][i],state->x[5][i],state->x[7][i]);
            }
...
static void F8(JHCtx *state)
{
      vuint32 i;

      /*xor the 512-bit message with the first half of the 1024-bit hash state*/
      for (i = 0; i < 16; i++)  state->x[i >> 2][i & 3] ^= ((vuint32*)state->buffer)[i];

      /*perform 42 rounds*/
      E8(state);

      /*xor the 512-bit message with the second half of the 1024-bit hash state*/
      for (i = 0; i < 16; i++)  state->x[(i+16) >> 2][i & 3] ^= ((vuint32*)state->buffer)[i];
}
...
JH_HashReturn JH_Update (JHCtx *state, const JH_BitSequence *data, JH_DataLength databitlen) {
  ...
  if (state->datasize_in_buffer > 0 && state->datasize_in_buffer+databitlen < 512) {
    memcpy(state->buffer+(state->datasize_in_buffer>>3), data, 64-(state->datasize_in_buffer>>3)+!!(databitlen&7));
  ...
  for (; databitlen >= 512; index = index+64, databitlen -= 512) {
  ...
  if (databitlen > 0) {
    memcpy(state->buffer, data+index, ((databitlen&0x1ff)>>3)+!!(databitlen&7));
    state->datasize_in_buffer = databitlen;
  }
...
JH_HashReturn JH_Final(JHCtx *state, JH_BitSequence *hashval) {
  if ((state->databitlen&0x1ff) == 0) {
    ...
    for (unsigned i = ((state->databitlen&0x1ff)>>3)+!!(state->datasize_in_buffer&7); i < 64; ++i) state->buffer[i] = 0;
  ...
  switch (state->hashbitlen) {
    case 224: memcpy(hashval, (vuint8 *)state->x+64+36, 28); break;
    case 256: memcpy(hashval, (vuint8 *)state->x+64+32, 32); break;
    case 384: memcpy(hashval, (vuint8 *)state->x+64+16, 48); break;
    case 512: memcpy(hashval, (vuint8 *)state->x+64+0,  64); break;
    default: return JH_BAD_HASHLEN;
  }
...
JH_HashReturn JH_Hash (JH_HashSize hashbitlen, const JH_BitSequence *data, JH_DataLength databitlen, JH_BitSequence *hashval) {
  JHCtx state;
  JH_HashReturn res = JH_Init(&state, hashbitlen);
  if (res != JH_SUCCESS) return res;
  res = JH_Update(&state, data, databitlen);
  if (res != JH_SUCCESS) return res;
  return JH_Final(&state, hashval);
}
